...
<div class="row">
    <div class="d-flex justify-content-end">
        @canany(['manage.jobs', 'create.jobs'])
            <a class="btn btn-primary btn-sm mb-1 mt-0" href="{{ route('jobs.index') }}">
                <i class="bx bx-list-ul me-1"></i> Job Posts
            </a>
        @endcanany
    </div>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">New Job Post</h4>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <form action="{{ route('jobs.store') }}" method="post">
                                ...
                                <select name="job_sector_id"
                                    ...
                                    @foreach ($jobsectors as $jobsector)
                                        <option value="{{ $jobsector->id }}"
                                            {{ old('job_sector_id') == $jobsector->id ? 'selected' : '' }}>
                                            {{ $jobsector->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('job_sector_id') <div class="text-danger">{{ $message }}</div> @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-bold">No. To Be Employed <span class="text-danger">*</span></label>
                                <input class="form-control @error('no_to_employed') is-invalid @enderror"
                                       type="number" name="no_to_employed"
                                       value="{{ old('no_to_employed') }}"
                                       placeholder="5" required>
                                @error('no_to_employed') <div class="text-danger">{{ $message }}</div> @enderror
                            </div>

                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-bold">Job Opening Date <span class="text-danger">*</span></label>
                                    <input class="form-control @error('job_opening_date') is-invalid @enderror"
                                           type="date" name="job_opening_date"
                                           value="{{ old('job_opening_date') }}" required>
                                    @error('job_opening_date') <div class="text-danger">{{ $message }}</div> @enderror
                                </div>

                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-bold">Job Closing Date <span class="text-danger">*</span></label>
                                    <input class="form-control @error('job_closing_date') is-invalid @enderror"
                                           type="date" name="job_closing_date"
                                           value="{{ old('job_closing_date') }}" required>
                                    @error('job_closing_date') <div class="text-danger">{{ $message }}</div> @enderror
                                </div>
                            </div>
